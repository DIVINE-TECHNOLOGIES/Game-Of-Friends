...
export const handler: Handler = async (event) => {
  ...
  let body: SignupBody = {};
  try {
    body = event.body ? (JSON.parse(event.body) as SignupBody) : {};
  } catch {
    return { statusCode: 400, body: JSON.stringify({ error: 'Invalid JSON' }) };
  }

  const { username, partnerUsername, password, nickname, partnerNickname, avatar, partnerAvatar, favoriteGame } = body;
  ...
  if (existsPartner) return { statusCode: 400, body: JSON.stringify({ error: `Friend's username "${partnerUsername}" is already taken!` }) };
